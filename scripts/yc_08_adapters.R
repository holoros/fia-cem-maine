## yc_08_adapters.R  (Yield-Curve Phase 2, step 4 — model adapters)
##
## Convert maine_yield_curves_v3_long.csv into four model-specific input
...
##   CEM:          cem_productivity_multipliers.csv — per-cell scaling
##                 factor for the CEM pipeline's growth multiplier,
##                 derived from the empirical asymptote relative to
##                 the population mean.
##   Woodstock:    woodstock_yields.csv — periodic AGB and merch volume
...
## Outputs: yield_curves/adapters/{gcbm,landis,cem,woodstock}_*.csv

args <- commandArgs(trailingOnly = TRUE)
yc_dir <- if (length(args) >= 1) args[1] else file.path(Sys.getenv("HOME"), "yield_curves")
out_dir <- file.path(yc_dir, "adapters")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

curves <- read.csv(file.path(yc_dir, "maine_yield_curves_v3_long.csv"),
                   stringsAsFactors = FALSE)
fits   <- read.csv(file.path(yc_dir, "maine_yield_curves_v3_fits.csv"),
                   stringsAsFactors = FALSE)
...
if (nrow(agb) > 0 && nrow(vol) > 0) {
  ...
  gcbm <- gcbm[, c("classifier_set","ft_group","ecoregion","owner",
                   ...
                   "total_above_kgC_m2")]
  write.csv(gcbm, file.path(out_dir, "gcbm_growth_curves.csv"),
            row.names = FALSE)
  cat(sprintf("Wrote gcbm_growth_curves.csv (%d rows)\n", nrow(gcbm)))
}
...
landis <- landis[, c("cell_key","ft_group","ecoregion","owner",
                      ...
                      "n_plots")]
write.csv(landis, file.path(out_dir, "landis_biomass_parameters.csv"),
          row.names = FALSE)
cat(sprintf("Wrote landis_biomass_parameters.csv (%d rows)\n", nrow(landis)))

# Also emit a LANDIS-style text block (one per cell × treatment)
landis_txt <- file.path(out_dir, "landis_biomass_parameters.txt")
...
writeLines(">> Source: maine_yield_curves_v3_fits.csv\n", con)
...
close(con)
cat(sprintf("Wrote landis_biomass_parameters.txt\n"))
...
write.csv(cem, file.path(out_dir, "cem_productivity_multipliers.csv"),
          row.names = FALSE)
cat(sprintf("Wrote cem_productivity_multipliers.csv (%d rows; mean a = %.1f)\n",
            nrow(cem), mean_a))
...
ws_out <- ws[, c("stratum","ft_group","ecoregion","owner","treatment",
                  ...
                  "CarbonMgHa")]
write.csv(ws_out, file.path(out_dir, "woodstock_yields_long.csv"),
          row.names = FALSE)
cat(sprintf("Wrote woodstock_yields_long.csv (%d rows)\n", nrow(ws_out)))
...
write.csv(ws_wide_agb, file.path(out_dir, "woodstock_yields_AGB_wide.csv"),
          row.names = FALSE)
cat(sprintf("Wrote woodstock_yields_AGB_wide.csv (%d rows)\n",
            nrow(ws_wide_agb)))
