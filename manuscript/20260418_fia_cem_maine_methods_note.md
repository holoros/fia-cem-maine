...
Forest Inventory and Analysis (FIA) data underpin the Resources Planning Act (RPA) Assessment's state-scale carbon projections, but state agencies and climate councils typically need projections that reflect climate change, state-specific market conditions, and landowner-specific management behavior on policy-relevant horizons. We extend the coarsened exact matching (CEM) approach of Van Deusen and Roesch (2013) with the stratification and transition refinements of Wear and Coulston (2019, 2025), layer a Maine-specific economic harvest module driven by 10 years of Maine Forest Service county stumpage and Silvicultural Activities Reports, add a Bayesian Reineke maximum-stand-density cap, episodic disturbance (spruce budworm, wind, fire), Potter (2017) species climate vulnerability, and ownership-class harvest behavior derived from the Harris-Caputo-Butler (2025) forest ownership raster. We project five harvest scenarios on a 1999 baseline through 2074 (15 five-year cycles × 100 bootstrap simulations) under HadGEM2-AO RCP 4.5 and RCP 8.5 climate forcing on OSC Cardinal HPC and cross-validate against subject-matched FIA panels at five-year intervals. Three findings stand out. First, the canonical refined pipeline (r17 in our progression nomenclature) yields a Maine 2004 total carbon estimate of 1,131 MMT, of which 62 percent is soil organic and 20 percent is above-ground live tree biomass; under business-as-usual harvest, RCP 4.5 wear+econ projects above-ground live tree carbon to 30 MMT by 2074 (an 87 percent decline). Second, owner-class harvest stratification (NIPF 0.5×, Industrial 1.5×, Tribal/Federal 0.2×, State 0.5×, Local 0.3× relative to uniform) is the largest single calibration improvement we identify: hindcast RMSE drops from 22.4 to 14.5 MMT and bias from −20 to near zero across all four RCP × overlay combinations. Owner-class behavior is therefore a first-order driver of state-scale carbon outcomes that uniform-rate models miss. Third, the harvest sensitivity collapses to a 14 MMT 2074 spread between BAU and +50 percent biomass once the SDImax cap and disturbance module are active, indicating that Reineke's self-thinning ceiling and stochastic mortality together set the dominant carbon ceiling in the refined pipeline. The pipeline is reproducible, open-source, and extensible to other states with comparable FIA, harvest reporting, and ownership raster infrastructure.
...
A Maine-focused CEM pipeline that combines Wear and Coulston (2019, 2025) stratification, multi-pool carbon accounting, county stumpage economic overlay, Bayesian Reineke maximum-stand-density cap, episodic disturbance, Potter (2017) species climate vulnerability, and Harris-Caputo-Butler (2025) landowner stratification produces calibrated, climate-consistent, behaviorally realistic carbon projections suitable for state-scale planning.

The dominant calibration finding is that owner-class harvest behavior is a first-order driver of state-scale outcomes. Recognizing the asymmetry between NIPF and Industrial harvest behavior reduces hindcast RMSE by 35 percent (22.4 to 14.5 MMT) and brings absolute bias from 15-22 MMT down to 0-7 MMT against subject-matched observed FIA inventory. Per-plot owner stratification using the HCB 2025 raster is therefore a more consequential refinement than the SDImax cap, climate decoupling, or species vulnerability layers individually, despite each being well-justified physically.

Three policy implications follow. First, state-scale carbon projections that ignore owner-class behavior systematically underestimate carbon retention in NIPF-dominated regions and overestimate retention in industrial-dominated regions. Second, owner-targeted policy levers (BPL light-management practice, conservation easement programs, Farm Bill payments to NIPF, mill-supply contracts with industrial owners) act on dimensions that uniform-rate projections cannot quantify. Third, cross-model ensembles (e.g., the four-model PERSEUS effort that motivated this refinement) should adopt a shared owner-class harvest behavioral parameter set to eliminate this source of cross-model disagreement.

The pipeline is reproducible, open-source (Apache 2.0, see fia-cem-maine repository), and extensible to other states with comparable FIA, harvest reporting, and ownership raster infrastructure. Future work includes per-cell yield-curve fitting (Phase 1.5 stratification table built; FVS factorial pending), a county × owner interaction model, and CMIP5 climate ensemble sampling for proper climate uncertainty propagation.
