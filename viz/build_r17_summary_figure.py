...
def load_run(rcp, econ):
    tag_path = SUM / f"state_rcp{rcp}_hadgem2_wear{'_econ' if econ else ''}_r17_ci.csv"
...
def main():
    ...
    fig.suptitle("Maine forest carbon trajectories under climate × harvest scenarios "
                 "(r17, refined pipeline)", fontsize=13, weight="bold")
    plt.tight_layout()
    out_path = OUT / "fig_r17_summary_2x2.png"
    plt.savefig(out_path, dpi=200, bbox_inches="tight")
    print(f"Wrote {out_path}")

    if summary_rows:
        sdf = pd.DataFrame(summary_rows)
        sdf.to_csv(OUT / "r17_summary_2x2.csv", index=False)
        print("\n=== r17 endpoint table ===")
