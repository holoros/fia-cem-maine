...
## Final result 2026-05-21: California donor hindcast confirms the null

The corrected California donor production hindcasts landed and confirm the null. The Washington residual against the 2019 EVALID 531900 cycle 4 comparison is minus 77.2 MMT under RCP 4.5 (projected 238.0 against observed 315.1) and minus 77.4 MMT under RCP 8.5 (projected 237.7). The neighbor pool baseline was minus 79.4 and minus 81.6. The projected subject carbon is essentially unchanged at the twenty year horizon (238.0 versus the baseline 237.6), so the small movement in the residual traces to the observed subject pool rather than to any real change in the projection. Adding a correctly loaded California donor pool does not fix Washington underprediction; the bias remains near minus 25 percent.

This closes the CONUS donor thread. Neither the ecoregion matching key alone, nor naive donor state list expansion, nor a properly loaded California donor pool reduces Washington bias. The binding constraint is donor growth rate composition, which is what the productivity matching work (asymptotic AGB stratification using the in house CONUS BGI surface) is now testing directly.
